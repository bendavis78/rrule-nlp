...
var TYPES = extend(this.CONTENT_TYPES, {
...
function Tokenizer(text) {
  ...
  for (c in text) {
    for (type in TYPES) {
      if (TYPES[type].test(text[c])) {
        this._tokens.push(new Token(text[c], text, type));
        break;
      }
    }
  }
 ...
 */
function RecurringEvent(nowDate, preferredTimeRange) {
...
RecurringEvent.prototype = {
  ...
  parse: function(s) {
    ...
    s = normalize(s);
    var event = this.parseStartAndEnd(s);
    if (!event) {
      return false;
    }
    this.isRecurring = this.parseEvent(event);
    if (this.isRecurring) {
    ...
    var date = this.parseDate(s);
    if (date) {
      return this.parseTime(s, date) || date;
    }

    // maybe we have a simple time expression
    return this.parseTime(s, this.nowDate);
  },
  ...
  parseEvent: function(s) {
    ...
    // recurring phrases
    if (t.hasType('every') || t.hasType('recurring_unit')) {
      if (t.containsText('every other')) {
        this.interval = 2;
      } else {
        this.interval = 1;
      }
...
window.parseRecurringEvent = function(s) {
  var r = new RecurringEvent();
  return r.parse(s);
};
